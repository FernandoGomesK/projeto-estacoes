...
function calcula_estacao()
{
    ...
    const imgEstacoes = document.querySelectorAll(".estacoes_img .estacao");

    imgEstacoes.forEach(div => {
      div.style.opacity = "0.3";
    });

    const seletorEstacaoAtual = ".estacao_img .estacao.${estacao}";
    const divEstacaoAtual = document.querySelector(seletorEstacaoAtual);

    if (divEstacaoAtual) {
      divEstacaoAtual.style.opacity = "1";
    };
